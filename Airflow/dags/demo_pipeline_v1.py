...
def cleanup_old_docs():
    docs_root = os.path.join(GE_ROOT, "uncommitted", "data_docs", "local_site")
...
def generate_data_docs():
    ...
    index_page_url = sites.get("local_site")
    print("Data Docs index:", index_page_url)

    docs_root = index_page_url.replace("file://", "").replace("index.html", "")
    count = 0
    for root, _, files in os.walk(docs_root):
        for name in files:
            if name.endswith(".html") and "validations" in root:
                count += 1
    print("Validation HTML files found:", count)
